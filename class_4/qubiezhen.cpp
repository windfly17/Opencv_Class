...
int main() {
    cv::Mat src = imread("D:\\Desktop\\Opencv_Class\\class_4\\qubiezhen.jpg");
    ...
    cv::dilate(src_th, src_pz, element);
    cv::erode(src_pz, src_close, element);

    cv::erode(src_th, src_fs, element);
    cv::dilate(src_fs, src_open, element);
